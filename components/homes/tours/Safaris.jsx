...
import { fetchSafaris, getTourImageSource } from "@/utils/tourPackages";
...
export default function Safaris() {
  ...
  useEffect(() => {
    const loadTours = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetchSafaris();
        ...
        setError("Error loading safaris");
      } finally {
        setLoading(false);
      }
    };
        ...
        try {
          ...
          if (response && response.success) {
            const filteredTours = response.data.filter(
              (tour) => tour.group_type === travelStyle
            );
  ...
  if (!filtered || filtered.length === 0) {
    return <div className="text-center py-40">No safaris found</div>;
  }

  return (
    <section className="layout-pt-xl layout-pb-xl bg-white-1">
